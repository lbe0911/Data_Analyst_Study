...
주요 분포 | 이항분포, 초기하분포, 포아송분포 | 정규(Z)분포, t 분포, 카이제곱(X^2)분포, F 분포, 지수분포
...
### 5.3 연속확률분포
**연속확률분포** = 연속적인 공간 안 확률변수가 가질 수 있는 값이 무한히 많기에, 특정한 값을 가질 확률이 0으로 수렴. 자신의 키가 175cm라고 했던 사람의 키가 사실은 정확한게 아닌 175.1234..cm인 것과 비슷한 이치라고 생각하자.<br>
연속확률분포는 확률변수 X가 특정구간에 속할 확률을 구해야 함.
- ex) 만약 10,000 명이 속한 집단 A의 키를 조사 10cm 단위로 히스토그램을 그리면 다음과 같은 형태가 나온다.
<p align="center"><img src="./image/10cm.png" width="100%" height="10%"></p>

이를 좀 더 세분화하여 1cm 단위로 히스토그램을 그리면 아래와 같이 부드럽게 보일 수 있다.
<p align="center"><img src="./image/1cm.png" width="100%" height="10%"></p>

점점 더 세분화하여 1mm, 0.001mm 단위로 쪼개지다 보면 아래와 같은 곡선 형태의 분포가 만들어진다.
<p align="center"><img src="./image/연속확률의.png" width="100%" height="10%"></p>

이러한 분포를 연속확률분포 또는 확률밀도곡선이라 표현한다. 전체 분포에서 특정 구간인 a에서 b의 사건이 발생할 수 있는 확률을 구하기 위해 확률밀도함수를 이용. 확률밀도함수는 확률변수 X가 가질 수 있는 값과 그에 대응하는 확률을 함수나 표, 그래프로 표현한 것이다.<br>이산확률분포는 특정 값을 가질 확률을 구할 수 있어 확률질량함수를 통해 값의 확률을 구한다. 연속형 확률분포는 확률밀도함수를 통해 전체 분포 중 특정 면적을 적분을 통해 분리해 면적의 구간 안에서 사건이 발생할 확률을 계산한다. 정규분포와 지수분포를 통해 연속형 확률분포를 배워보자.
#### 5.3.1. 정규분포
**정규분포** 는 가장 대표적인 연속확률 분포. 통계를 잘 모르는 사람도 구릉 모양의 선으로 표현된 분포는 자주 접한다. 또 다른말로 수학자인 가우스가 증명하여 가우스 분포라 부르기도 한다. 어느 학교 한 학급의 학생 키 분포, 국내 30세 남성의 수면시간 분포 등 세상의 많은 데이터들은 정규분포로 이루어짐. 자연적인 발생 확률도 정규분포를 따르는 경우가 많다.<br> 이항분포를 가지는 동전던지기 예시를 통해 정규분포를 이해해보자.<br> 우선 동전을 10번씩 던져 앞면이 나오는 횟수, 이를 무수히 반복한다 가정해 보면 앞면이 4, 5, 6번 나오는 경우가 많을 것이고, 0번 혹은 10번 모두 앞면인 경우는 매우 드물 것. 이를 막대그래프로 나타내면 아래와 같다.
<p align="center"><img src="./image/동전을.png" width="100%" height="10%"></p>

정규분포와 매우 유사한 모양이다. 이번에는 횟수를 늘려 동전을 1,000번 던졌다고 가정해보면, 앞면이 500번가량 나온 경우가 많을 것이고, 극단적인 경우는 적을 것이다. 이를 그래프로 나타내면 부드러운 곡선 형태로 분포가 나타남. 이렇게 동전 던지기 횟수를 무한으로 늘리면 정규분포 형태가 만들어진다. 이는 확률이 동일하지 않은 찌그러진 동전의 경우라도, 중심 위치가 다를 뿐 마찬가지로 정규분포를 나타낸다. 이러한 원리를 통해 모집단에서 추출한 표본에 계산된 평균, 비율도 정규분포를 따르게 됌.<br> 정규분포가 성립하려면 몇 가지 조건을 만족해야 한다. 정규분포는 평균과 표준편차에 따라 형태가 조금씩 다름. 평균을 중심으로 좌우 대칭의 종 모양의 형태를 가짐. 그러므로 형균과 중앙값, 최빈값이 같음. 평균을 기준으로 좌우 1표준편차안에 68.26%의 확률변숫값이 포함된다.
- ex) 평균이 30이고 표준편차 5라면, 25 ~ 35 안에 약 68% 값 포함. 2표준편차 안에는 95.44%, 3표준편차 안에는 99.74%가 포함 앞에 표준편차의 경험법칙과 유사하다.

<p align="center"><img src="./image/정규분포.png" width="100%" height="10%"></p>

정규분포는 그래프의 면적을 계산해 확률을 구한다. 분포의 평균과 분산에 따라 형태에 차이가 나게되고, 다른 분포끼리 비교를 하고 면적을 계산하는 것은 복잡하고 어렴다. 이러한 불편을 줄이기 위해서 정규분포를 일괄적인 기준으로 표준화 시켜야 한다. 표준화된 정규분포를 표준정규분포라 한다. 각기 다른 정규분포간 비교의 동질성을 만들기 위해 평균 = 0, 분산 = 1인 표준정규분포의 형태로 변환을 해준다. 확률변수 X의 편차 $(X - \mu)$ 를 표준편차 $(\sigma)$ 로 나누어 확률변수를 표준화한 값 (Z)으로 변환하면 표준정규분포가 된다. 이를 통해 정규분포상의 구간확률을 쉽게 구하고 비교할 수 있다.
#### 6.3.2 지수분포
**지수분포** = 특정 사건이 발생한 시점에서 다음 사건이 발생할 때까지의 시간을 확률변숫값으로 하는 분포를 뜻함. 이산확률분포에서 다뤘던 포아송분포가 특정 사건이 발생하는 횟수를 나타낸 변수라면, 지수분포는 발생하는 사건 다음 사건이 일어날 때까지 대기 시간을 다루는것에 차이가 있다. 길이나 무게는 무한히 쪼갤 수 있어 시간도 특정 구간에 무한한 변숫값이 존재할 수 있으므로 시간을 나타내는 지수분포는 연속확률분포다.
- ex) A 가전업체의 A/S센터에 전화가 걸려온 후 다음 전화가 걸려올 때까지의 시간에 대한 확률변수를 그래프로 나타내면 아래의 형태가 될것이다.
<p align="center"><img src="./image/지수분포.png" width="100%" height="10%"></p>

지수분포 그래프에서 확률변수 X인 시간이 증가할수록 사건이 발생할 확률이 지수적으로 감소. 오랜 시간 지날수록 전화가 한번도 걸려오지 않을 확률은 매우 적어진다. 지수분포는 표준정규분포처럼 절대적 기준이 없어 평균에 따라 기울기가 정해진다. 포아송 분포와 밀접하게 연관되어있고, 포아송분포의 확률밀도함수 기호인 람다 $(\lambda)$ 를 사용한다.<br>
지수분포의 확률밀도함수는 $f(x)=\lambda e^{- \lambda x}$ 로 나타낸다.<br>
- x : 사건과 다음 사건의 시간의 확률변수
- e : 자연로그의 밑수(2.71828)
- $\lambda$ : 특정 시공간 안에서의 평균 사건 발생 횟수

포아송분포에서 일정 시간동안 평균적 발생한 사건의 횟수를 뜻했던 $\lambda$ 에 역수를 취하면 하나의 사건이 발생한 후, 다음 사건이 발생하기까지 평균 소요 시간이 된다. 평균을 제곱하면 분산이 되며, 분산에 제곱근을 취한 값인 표준편차는 평균과 동일하다. 정리하면 아래와 같다.<br>
- 지수분포의 (소요시간) 평균 : $\mu = {1 \over \lambda}$
- 지수분포의 (소요시간) 분산 : $\sigma ^2 = {1 \over \lambda ^2}$
- 지수분포의 (소요시간) 표준편차 : $\sigma = \sqrt{1 \over \lambda ^2}$

지수분포상의 실제 확률을 구하려면 어떻게? 확률 분포의 경우를 3가지로 나누어 상황에 맞게 적용해야 한다.<br>
1. t 시점 이전에 발생할 확률 $(1-e^{- \lambda x})$
2. t 시점 이후에 발생할 확률 $(e^{- \lambda x})$
3. t 시점과 t' 시점 사이에 발생할 확률 $(-e^{- \lambda x}) - (-e^{- \lambda x})$

<p align="center"><img src="./image/지수분포의.png" width="100%" height="10%"></p>

- ex) A가전업체의 A/S센터에 1시간당 평균 5회 전화가 걸려오면, 전화 간의 평균 시간 간격은 0.2 시간. 이는 $\lambda = 5$ 이고, $ {1 \over \lambda} = {1 \over 5} = 0.2$ 로 정리가 된다. 즉 포아송분포의 평균 $\lambda$ 는 5이고, 지수분포의 평균은 0.2가 되는 것이다. 이러한 조건에서 전화가 걸려온 후, 30분 이내에 전화가 올 확률은 $1-e^{-5 \times 0.5} =$ 1-0.08208 = 0.918으로 계산할 수 있다. 다시 말해 91.8%의 확률로 전화 후에 30분 안으로 또 전화가 올 것이라 해석 된다.

### 5.4 중심극한정리
실제 분석에 사용되는 데이터는 분포 형태가 다양, 편증이 심한 경우도 자주 나타난다. 통계적 추론과 검증을 위한 대부분의 분석 모델은 데이터가 정규분포라는 가정하에 이뤄지는데, 데이터는 최대한 정규분포와 일치하는 형태가 되어야 한다. 이 개념이 중심극한정리 이다.<br>
중심극한정리는 데이터의 크기(n)가 일정한 양(예 30개)을 넘으면, 평균의 분포는 정규분포에 근사하다는 이론 모집단으로부터 무작위로 표본을 여러 번 추출한 다음, 추출된 각각의 표본들의 평균을 분포로 그려보면 정규분포의 형태를 가진다는 것이다.<br>여기서 주의해야 할 점은 표본의 양이 충분하면, 표본의 평균이 모집단의 평균과 유사해진다는 뜻이 아니라는 것. 표본을 여러번 추출했을 때, '각각의 표본' 평균들의 분포가 정규분포를 이루는 것이다.<br> 
- ex) A지역에는 총 100가구가 살고 있다. 이 지역의 표본을 추출해 평균 가구 수익을 추정, 만약 한 가구만 표본으로 추출한다고 하면 총 100종류의 표본을 뽑을 수 있다. 추출한 100개의 표본들 각각의 평균을 분포로 그리면 A지역의 가구 수익 분포와 동일한 모양을 가진다. 10가구씩 추출한 표본들의 평균에 대한 분포는 모집단 평균을 기준 어느 정도 몰려 있는 완만한 종 모양을 보이고, 50가구씩 추출하면 평균은 어떤 분포를 보이겠나? 아래 그림과 같이 정규분포와 흡사한 형태를 보이게 된다. 만약 90가구씩 표본을 뽑는다면 각 표본은 모집단과 매우 유사하기 때문에 평균들은 별 차이가 없을 것이다. 표본평균의 분포는 바늘처럼 뾰족하게 가운데 모여 있는 모양이 된다.

<p align="center"><img src="./image/표본의.png" width="100%" height="10%"></p>

표본이 일정 수준 이상 크기가 되면 표본 평균의 분포가 정규분포에 가까워지게 되므로, 전수조사 없이도 표본 평균의 분포는 파악할 수 있다. 이를 통해 모평균의 신뢰구간을 쉽게 구할 수 있다. 일반적으로 표본의 크기가 30이상이면 표본 평균이 정규분포를 한다고 가정한다. 하지만 모집단의 분포가 극단적인 경우 더 많은 표본이 필요할 수 있어 되도록 표본의 수를 충분히 확보하자.<br> 중심극한저이는 통계학에 있어 추정과 가설검정을 위한 핵심적인 이론이다.<br>
모집단의 분포가 어떤 형태를 가지고 있는지 몰라도, 표본을 충분히 추출하면 표본 평균을의 분포가 정규분포를 이루기 때문에 통계적 추정이 가능해진다. 정규분포의 좌우 1시그마 포함 비중을 응용해 보면, 만약 추출한 표본의 평균 87이면 모수의 실제 평균 = 87 $\pm 1 \sigma$ 일 확률은 68%라고 추정 할 수 있다.<br>
이러한 원리를 통해 가설검정을 할 수 있고, 더 나아가 예측 모델링 등을 할 수 있다고 한다.
